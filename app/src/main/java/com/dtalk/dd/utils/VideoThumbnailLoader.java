...
public class VideoThumbnailLoader {
    ...
    public void display(String path, String url,ImageView iv,int width,int height,ThumbnailListener thumbnailListener){

        new ThumbnailLoadTask(path, url,iv,width,height,thumbnailListener).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);//使用AsyncTask自带的线程池
    ...
    private class ThumbnailLoadTask extends AsyncTask<Void, Void, Bitmap> {
        ...
        private int width;
        private int height;
        public ThumbnailLoadTask(String path, String url,ImageView iv,int width,int height,ThumbnailListener thumbnailListener){
            this.path = path;
            this.url = url;
            this.iv = iv;
            this.width = width;
        ...
        protected Bitmap doInBackground(Void... params) {
            ...
            if (!TextUtils.isEmpty(url)) {
                ...
                if (bitmap == null||bitmap.isRecycled()) {
                    ...
                    File file = new File(path);//创建文件,这里由于项目原因,我就随便写一个,实际情况不是这样,大家留意一下
                    if (null != file && file.exists()) {//去磁盘缓存取
                        ...
                        if (null==bitmap) {
                            ...
                            //将图片保存到磁盘文件,作为缓存
                            try {
                                ImageUtils.saveImageToSD(file, bitmap, 100);
                            }
                            catch (Exception e) {

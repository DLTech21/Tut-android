...
import com.dtalk.dd.ui.adapter.ImageScaleAdapter;
import com.dtalk.dd.ui.widget.HackyViewPager;
import com.dtalk.dd.ui.widget.material.MaterialTextView;
import com.dtalk.dd.ui.widget.photoview.PhotoView;
import com.dtalk.dd.utils.EvaluateUtil;
    ...
    private void initDot(int index) {
        ...
        for (int i = 0; i < picDataList.size(); i++) {
            ...
            if (i == index || picDataList.size() == 1) {
                ...
                view.setBackgroundResource(R.drawable.type_normal);
            }
            // 指定点的大小
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(
                    ScreenUtil.instance(this).dip2px(5), ScreenUtil.instance(this).dip2px(5));
            // 指定点的间距
            layoutParams.setMargins(ScreenUtil.instance(this).dip2px(2), 0, ScreenUtil.instance(this).dip2px(2), 0);
            // 添加到线性布局中
...
//        计算出与屏幕的比例，用于比较以宽的比例为准还是高的比例为准，因为很多时候不是高度没充满，就是宽度没充满
        float h = ScreenUtil.instance(this).getScreenHeight() * 1.0f / intrinsicHeight;
        float w = ScreenUtil.instance(this).getScreenWidth() * 1.0f / intrinsicWidth;
        if (h > w) {
            h = w;
        } else {
            w = h;

...
import com.dtalk.dd.imservice.entity.MsgAnalyzeEngine;
...
public class ProtoBuf2JavaBean {
    ...
    public static UserEntity getUserEntity(IMBaseDefine.UserInfo userInfo){
        ...
        userEntity.setIsFriend(userInfo.getIsFriend());
    ...
    public static SessionEntity getSessionEntity(IMBaseDefine.ContactSessionInfo sessionInfo){
        ...
        String desMessage = new String(Base64.decode(Security.getInstance().DecryptMsg(content), Base64.DEFAULT));
        // 判断具体的类型是什么
    ...
    */
    ...
    public static MessageEntity getMessageEntity(ApplicantEntity applicantEntity){

        MessageEntity messageEntity = null;
        IMBaseDefine.MsgType msgType = Java2ProtoBuf.getProtoMsgType(applicantEntity.getMsgType());
        byte[] sendContent = applicantEntity.getContent().getBytes();
        ByteString bytes = ByteString.copyFrom(sendContent);
        IMBaseDefine.MsgInfo msgInfo = IMBaseDefine.MsgInfo.newBuilder()
                .setMsgData(bytes)
                .setMsgId(applicantEntity.getMsgId())
                .setMsgType(msgType)
                .setCreateTime(applicantEntity.getCreated())
                .setFromSessionId(applicantEntity.getFromUserId())
                .build();
        ...
        if(messageEntity != null){
            messageEntity.setToId(applicantEntity.getToSessionId());
        }
    ...
    public static int getJavaMsgType(IMBaseDefine.MsgType msgType){
        switch (msgType){
            ...
            case MSG_TYPE_GROUP_VIDEO:
                return DBConstant.MSG_TYPE_GROUP_VIDEO;
            case MSG_TYPE_GROUP_URL:
                return DBConstant.MSG_TYPE_GROUP_URL;
            case MSG_TYPE_GROUP_FILE:
                return DBConstant.MSG_TYPE_GROUP_FILE;
            case MSG_TYPE_GROUP_EMOTION:
                return DBConstant.MSG_TYPE_GROUP_EMOTION;
            ...
            case MSG_TYPE_SINGLE_VIDEO:
                return DBConstant.MSG_TYPE_SINGLE_VIDEO;
            case MSG_TYPE_SINGLE_URL:
                return DBConstant.MSG_TYPE_SINGLE_URL;
            case MSG_TYPE_SINGLE_FILE:
                return DBConstant.MSG_TYPE_SINGLE_FILE;
            case MSG_TYPE_SINGLE_EMOTION:
                return DBConstant.MSG_TYPE_SINGLE_EMOTION;
            default:

...
import com.dtalk.dd.utils.VideoThumbnailLoader;
...
public class MediaPreviewActivity extends BaseActivity implements
	...
	private boolean prepareActivity() {
		...
		if (StringUtils.isNotEmpty(mVideoPath)) {
			mCoverPath = mVideoPath.replace(".mp4", ".png");
		}
			...
			protected File doInBackground(Void... params) {
				...
				mAuthorBitmapPath = ThemeHelper
						.updateVideoAuthorLogo(
								mThemeCacheDir,
								getString(R.string.record_camera_author, "Film by "+ IMLoginManager.instance().getLoginInfo().getMainName()), false);
				return result;
			}
...
//			if(!file.exists())
//				file.createNewFile();
//			boolean isThumb = FFMpegUtils.captureThumbnails(mVideoPath, mCoverPath, "80");
//		}catch (Exception e) {
//
//		}
		VideoThumbnailLoader.getIns().display(mCoverPath, mVideoPath, null, 200, 200, new VideoThumbnailLoader.ThumbnailListener() {
			@Override
			public void onThumbnailLoadCompleted(String url, ImageView iv, Bitmap bitmap) {
				hideProgress();
				mStartEncoding = false;
				finish();
				startActivity(new Intent(getApplicationContext(), VideoPlayerActivity.class).putExtra(
						"path", mVideoPath).putExtra(
						"cover_path", mCoverPath));
			}
		});
